...
*   **WebP 자동 변환**: 포함된 Node.js 스크립트(`optimize-images.js`)를 통해 원본 이미지(`png`, `jpg`, `jpeg`)를 무손실 WebP 형식으로 자동 최적화하고 **원본 파일을 삭제**하여 저장 용량을 최소화합니다.
...
3.  최적화 스크립트 실행:
    ...
    ```
    이 스크립트를 실행하면 추가된 `png`, `jpg`, `jpeg` 파일들이 WebP로 변환되고 원본 파일은 삭제됩니다.

## CDN URL 사용 예시

jsDelivr를 통해 에셋에 접근할 수 있습니다.
*   형식: `https://cdn.jsdelivr.net/gh/사용자명/Pokopia-Info-CDN@main/폴더명/파일명.webp`
